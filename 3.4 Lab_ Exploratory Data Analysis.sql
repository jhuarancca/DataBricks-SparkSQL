...
Round(quantity*unitPrice,2) as totalAmount,countryName
FROM outdoorProducts
WHERE quantity>0;
...
SELECT countryName,Sum(totalAmount)
FROM sales
...
SELECT countryName, Sum(quantity) as totalQuantity
FROM outdoorProducts
WHERE quantity>0
GROUP BY countryName;
...
CREATE TABLE countryCodes USING PARQUET 
OPTIONS (
